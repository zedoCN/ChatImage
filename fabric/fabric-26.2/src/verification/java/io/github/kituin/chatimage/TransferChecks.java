...
public final class TransferChecks {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String legacy = "file:///D:\\MineCraft\\截图 测试\\100%.png";
        check(LocalImagePaths.normalizeUri(legacy).getPath().equals("/D:/MineCraft/截图 测试/100%.png"), "Legacy Windows clipboard URL normalizes backslashes");
        Path specialPath = Path.of("截图 100%,[测试].png").toAbsolutePath();
        String escaped = LocalImagePaths.toUrl(specialPath);
        check(!escaped.contains(",") && !escaped.contains(" ") && !escaped.contains("["), "CICode path delimiters escaped");
        check(LocalImagePaths.parse(escaped).equals(specialPath), "Escaped Unicode and percent filename round trip");
        check(LocalImagePaths.normalizeUri("file:///D:/Screenshots/a%20b.png").getPath().equals("/D:/Screenshots/a b.png"), "Existing escaped Windows URL unchanged");
        String firstLocal = LocalImagePaths.attachment(Path.of("first.png"));
        check(LocalImagePaths.parse(firstLocal).equals(Path.of("first.png").toAbsolutePath()), "Short attachment resolves original path");
        MultiImageMessage pair = new MultiImageMessage("[[CICode,url=" + firstLocal + "]] [[CICode,url=file:///second.png]]");
        check(pair.size() == 2 && pair.fits(), "Two image message fits vanilla chat");
        check(pair.render(List.of("A", "B")).equals("[[CICode,url=A]] [[CICode,url=B]]"), "Multi image replacement preserves wrappers and order");
        check(!new MultiImageMessage("file:///a.png file:///b.png file:///c.png").fits(), "Oversized batch rejected before uploading");
        check(new MultiImageMessage("before file:///a.png after").render(List.of("R")).equals("before [[CICode,url=R]] after"), "Multi image text preserved");
