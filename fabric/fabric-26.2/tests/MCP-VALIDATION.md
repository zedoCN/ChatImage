...
## 2026-09-08：port.2 本地化修复

重新构建成功，通过 MCP 确認正式 port.2 JAR 的类来源。运行时切换 zh_cn 并重载资源，分类标题为“聊天栏图片显示”，End 功能名称为“打开配置文件设置”；英文两处也正确解析。原 HMCL 安装包已更新，port.1 保留为 `.jar.disabled`。
